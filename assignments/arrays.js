...
let lastCar = 0;
lastCar =   `${inventory[49]["car_make"] + " " + inventory[49]["car_model"]}`;
...
for (let p = 0; p < inventory.length; p++) {
    carYears.push(inventory[p]["car_year"]);
};
...
let oldCars = carYears.filter( item => {
    return item < 2000 ;
});
...
for ( b = 0 ; b < inventory.length; b++) {
    if(inventory[b]["car_make"] === "Audi" || inventory[b]["car_make"] === "BMW" ) {
